...
{
    public class NewCategoryCommandTests
    {
        ...
        public async Task ExecuteCommandTest(long chatId)
        {
            ...
            var command = new NewCategoryCommand("/test", "test", configuration,
        ...
        public async Task RemoveChatIdTest(long chatId)
        {
            ...
            Assert.AreEqual(0, command.CategoryFromUser.Count);
            Assert.That(command.CategoryFromUser.ContainsKey(chatId) == false);            
        }
        ...
        public async Task SaveInfoFromUser_StateNull_Test(long chatId, string message)
        {
            ...
            inputState.Setup(x => x.ChangeState(It.IsAny<IUniqueChatId>(), It.IsAny<string>()))
                                  .Callback(() => 
                                  {
                                      command.SetCategoryName(chatId, "test");
            ...
            operationMock.Verify(x => x.CreateNewCategory(It.IsAny<long>(), It.IsAny<NewCategory>(), It.IsAny<IConfiguration>()), Times.Once);
        }
    }
}